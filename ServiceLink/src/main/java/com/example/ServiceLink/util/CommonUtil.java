...
public class CommonUtil {
    ...
    public void validateEmail(UserRequestDTO userRequestDTO) {
        ...
        if (email != null && !email.isEmpty()) {
            ...
            String emailContent = "<div style=\"font-family: Arial, sans-serif; font-size: 14px; color: #333;\">" + "<p>Hi " + userRequestDTO.getName() + ",</p>" + "<p>Your OTP for verification is:</p>" + "<div style=\"display: flex; justify-content: center;align-items: center;\">" + "<div style=\"border: 1px solid #a5a5a8; padding: 10px; font-size: 24px; font-weight: bold; color: #4CAF50; text-align: center; width: 50px; height: 50px; margin: 0 5px;\">" + otp.charAt(0) + "</div>" + "<div style=\"border: 1px solid #a5a5a8; padding: 10px; font-size: 24px; font-weight: bold; color: #4CAF50; text-align: center; width: 50px; height: 50px; margin: 0 5px;\">" + otp.charAt(1) + "</div>" + "<div style=\"border: 1px solid #a5a5a8; padding: 10px; font-size: 24px; font-weight: bold; color: #4CAF50; text-align: center; width: 50px; height: 50px; margin: 0 5px;\">" + otp.charAt(2) + "</div>" + "<div style=\"border: 1px solid #a5a5a8; padding: 10px; font-size: 24px; font-weight: bold; color: #4CAF50; text-align: center; width: 50px; height: 50px; margin: 0 5px;\">" + otp.charAt(3) + "</div>" + "<div style=\"border: 1px solid #a5a5a8; padding: 10px; font-size: 24px; font-weight: bold; color: #4CAF50; text-align: center; width: 50px; height: 50px; margin: 0 5px;\">" + otp.charAt(4) + "</div>" + "<div style=\"border: 1px solid #a5a5a8; padding: 10px; font-size: 24px; font-weight: bold; color: #4CAF50; text-align: center; width: 50px; height: 50px; margin: 0 5px;\">" + otp.charAt(5) + "</div>" + "</div>" + "<p>Please use this OTP to complete your verification process. The OTP is valid for 5 minutes.</p>" + "<p>If you did not request this OTP, please ignore this email.</p>" + "<hr>" + "<p style=\"font-size: 12px; color: #777;\">" + "<i>Disclaimer: The information contained in this email is confidential and intended only for the recipient. If you are not the intended recipient, you are advised that any use, dissemination, distribution, or reproduction of this email is prohibited. If you have received this email in error, please notify us immediately by return email and delete this email and any attachments from your system.</i>" + "</p></div>";
    ...
    public boolean verifyOtp(String email, String otp) {
        ...
        if (otpEntity != null && otpEntity.getOtp().equals(otp) && otpEntity.getExpirationTime().isAfter(LocalDateTime.now())) {
            ...
            User user = userRepository.findByEmailAndStatus(email, User.Status.OTP_VERIFY.value).orElseThrow(() -> new BadRequestException(messageSource.getMessage("EMAIL_NOT_FOUND", null, Locale.ENGLISH)));
    ...
    public void validationAddUser() {
        String email = "stormhokspam@gmail.com";
        byte[] userStatus = {User.Status.SUPER_ADMIN.value};
        Optional<User> user1 = userRepository.findByEmailAndStatusIn(email, userStatus);
        if (user1.isEmpty()) {
            User user = new User();
            user.setName("stormhok");
            user.setEmail(email);
            String password = "password123";
            user.setPassword(passwordEncoder.encode(password));
            user.setStatus(User.Status.SUPER_ADMIN.value);
